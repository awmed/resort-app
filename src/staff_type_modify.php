...
if(isset($_SESSION['deleted'])) {
    ...
    '<script>                     
        setTimeout(function() {
            swal({
                title: "Success",
                text: "Staff Type Deleted Successfully !",
                type: "success"
            })
        }, 100);
    </script>';
...
if(isset($_SESSION['updated'])) {
    ...
    '<script>                     
        setTimeout(function() {
            swal({
                title: "Success",
                text: "Staff Type Updated Successfully !",
                type: "success"
            })
        }, 100);
    </script>';
...
<?php require 'inc/_global/views/head_start.php'; 
	echo '<title> MODIFY STAFF TYPE </title>'
?>
...
<div class="content">
    <nav class="breadcrumb bg-white push d-print-none">
        ...
        <span class="breadcrumb-item active"> MODIFY STAFF TYPE </span>
    </nav>
    <div class="block">
        <div class="block-header block-header-default">
            <h2 class="block-title d-print-none" style="font-size:25px"><strong> MODIFY STAFF TYPE </strong></h2>
                        ...
                        try {
                            ...
                            if(!@mysqli_stmt_prepare($stmt, $sql)) {
                                ...
                                while($row=mysqli_fetch_array($result)) {
                                    echo ('
                                    <tr>
                                        <td class="text-center">'.$row["s_type_id"].'</td>
                                        ...
                                        <td class="text-center d-print-none">
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="Edit">
                                                    <a href="staff_type_edit.php?ID='.$row['s_type_id'].'&type='.$row['s_type'].'&desc='.$row['description'].'"> <i class="fa fa-pencil"></i> Edit</a>
                                                </button>
                                                <button type="button" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="Delete">
                                                    <a href="inc/backend/database/staff_type_edit.php?ID='.$row['s_type_id'].'"> <i class="fa fa-times"></i> Delete</a> 
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
