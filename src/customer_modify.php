...
<?php require 'inc/_global/views/head_start.php'; 
	echo '<title> MODIFY CUSTOMER </title>'
?>
...
<div class="content">
    <nav class="breadcrumb bg-white push d-print-none">
        ...
        <span class="breadcrumb-item active"> MODIFY CUSTOMER </span>
    </nav>
    <div class="block">
        <div class="block-header block-header-default">
            <h2 class="block-title d-print-none" style="font-size:25px"><strong> MODIFY CUSTOMER </strong></h2>
                            ...
                            if(!@mysqli_stmt_prepare($stmt, $sql)) {
                                ...
                                while($row=mysqli_fetch_array($result)) {
                                    ...
                                    <tr>                                        
                                        ...
                                        if($status!=1) {
                                            echo '<img class="" src="assets/uploads/customer-image/default.jpg" alt="" style="border-radius:80px;width:50px;height:50px;">';
                                        }
                                        if($status==1) {
                                            $imagename = "assets/uploads/customer-image/customer_id-".$ID."*"; //select the customer image with any image extension
                                            $imageinfo = glob($imagename);
                                            $imageExt = explode(".", $imageinfo[0]);
                                            $imageActualExt = $imageExt[1];
                                            echo '<img class="" src="assets/uploads/customer-image/customer_id-'.$ID.'.'.$imageActualExt.'?'.mt_rand().' " alt="" style="border-radius:80px;width:50px;height:50px;object-fit:cover;">';
                                        ...
                                        <td class="text-center d-print-none">
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="View Profile">
                                                    <a href="customer_profile.php?ID='.$row['customer_id'].'&first='.$row['f_name'].'&last='.$row['l_name'].'&email='.$row['email'].'&gender='.$row['gender'].'&nationality='.$row['nationality'].'&phone='.$row['phone'].'&address='.$row['address'].'&status='.$row['img_status'].'"> <i class="fa fa-user"></i> </a>
                                                </button>
                                                <button type="button" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="Edit">
                                                    <a href="customer_edit.php?ID='.$row['customer_id'].'&first='.$row['f_name'].'&last='.$row['l_name'].'&email='.$row['email'].'&gender='.$row['gender'].'&nationality='.$row['nationality'].'&phone='.$row['phone'].'&address='.$row['address'].'&status='.$row['img_status'].'"> <i class="fa fa-pencil"></i> Edit</a>
                                                </button>
                                                <button type="button" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="Delete">
                                                    <a href="inc/backend/database/customer_edit.php?ID='.$row['customer_id'].'&status='.$row['img_status'].'"> <i class="fa fa-times"></i> Delete</a>
                                                </button>                                                    
                                            </div>
                                        </td>
                                    </tr>
